...
class TestUserRegistration:
    ...
    def test_user_can_register_successfully(self, driver, base_url):
    ...
    def test_registration_with_invalid_email_shows_errors(self, driver, base_url):
        ...
        assert WebDriverWait(driver, 5).until(expected_conditions.visibility_of_element_located(AuthModalWindowLocators.EMAIL_ERROR_MESSAGE)).text == "Ошибка"

        # Проверка 2: поле Email выделено красным
        assert WebDriverWait(driver, 5).until(expected_conditions.visibility_of_element_located(AuthModalWindowLocators.EMAIL_INPUT_ERROR))

        # Проверка 3: поле «Пароль» выделено красным
        assert WebDriverWait(driver, 5).until(expected_conditions.visibility_of_element_located(AuthModalWindowLocators.PASSWORD_INPUT_ERROR))

        # Проверка 4: поле «Повторите пароль» выделено красным
        assert WebDriverWait(driver, 5).until(expected_conditions.visibility_of_element_located(AuthModalWindowLocators.PASSWORD_REPEAT_INPUT))


    def test_registration_existing_user_shows_errors(self, driver, base_url, existing_user_credentials):    
